...
- **ステータス**: **フェーズ1向け凍結（2026-09-09）**。`03` 第7章の未決事項12件は全件解決。以降の変更はフェーズ1スコープ内の誤り訂正・実装スパイク結果の反映に限る（残る先送り項目は §15）。
  ...
  - 2026-09-18 追補（テナント分離の不備を修正：他テナントの店舗データを操作できてしまう。
    ...
    （存在有無を漏らさない）を返すようにした。
  - 2026-09-18 追補（ホーム画面のグループ分け・「卓・注文」を「注文管理」に改称）：
    ホーム画面の機能カードを「管理業務」（店舗設定・営業日・決済手段・卓（テーブル・
    カウンター）・ユーザー管理・監査ログ）と「店舗営業」（予約・メニュー管理・注文管理）の
    2グループに分けて見出し付きで表示するようにした。`app_feature` に
    `group_key VARCHAR(20)`（`MANAGEMENT`／`OPERATIONS`。`V16__app_feature_groups.sql`）を
    追加し、グループ内の表示順が要望どおりになるよう `display_order` も振り直した
    （店舗設定→営業日→決済手段→卓→ユーザー管理→監査ログ→予約→メニュー管理→注文管理の
    9件で、グループをまたいでも一意な連番のまま）。`GET /api/v1/app-features` のレスポンスに
    `group` を追加し、フロント（`Home.tsx`）はこれで `features` を絞り込んで2つの見出し
    （`GROUP_LABELS`）ごとに描画する。あるロールにとって片方のグループの表示対象が0件の
    場合（例：ホールスタッフは「管理業務」の6件すべてが対象外）、その見出し自体を表示しない。
    あわせて `feature_key = 'floor'` の表示名を「卓・注文」から「注文管理」に変更した
    （`feature_key`・エンドポイント・権限は変更なし。画面側の見出し（`FloorPage.tsx`）も
    「注文管理」に統一）。実機で経営管理者・ホールスタッフ双方の見え方を確認した。
  - 2026-09-18 追補（注文管理の卓一覧を卓管理画面と揃える。FR-E01）：注文管理（`/floor`）の
    卓ボードが卓番号・エリアのみの表示で「卓のイメージがしにくい」との指摘を受け、卓管理画面
    （`/settings/tables`）と同じ情報・絞り込みに揃えた。カード表示に席種類（テーブル／
    カウンター）・席数を追加（例：「テーブル・席数4・空席」）し、一覧の上に「席種類で絞り込み」
    「有効/無効で絞り込み」の2つの検索条件を追加した（`TablesPage.tsx` の
    `seatTypeFilter`／`activeFilter` と同じ実装）。バックエンドの変更はなく（`GET
    /api/v1/stores/{storeId}/tables` は元々 `seatType`／`seatCount` を返していた）、
    `FloorPage.tsx` の表示のみの変更。
  - 2026-09-18 追補（注文管理の卓一覧、行の並べ方を調整。FR-E01）：直前の追補で1行にまとめて
    いた卓情報を、上段「卓番号・エリア・（無効）・席種類・席数」／下段「利用中／空席・利用人数・
    オープン時刻」の2段に分けた（卓自体の属性と、今の利用状況を行で区別する）。`FloorPage.tsx`
    の表示のみの変更（バックエンドは変更なし）。
  - 2026-09-18 追補（卓オープン画面：人数の初期値に選択した卓の席数を設定。FR-E01）：
    卓オープン画面の「人数」欄の初期値を、固定の2名から、選択した卓の `seatCount`
    （卓マスタの席数）に変更した（`seatCount = 0` の卓は1名を初期値とする）。スタッフが
    毎回入力し直す手間を減らすための表示のみの変更で、入力後の上書きは従来どおり可能
    （バックエンドは変更なし）。
  - 2026-09-18 追補（卓オープン画面：人数の初期値を席種類の既定値に変更。直前の追補を修正。
    FR-E01）：直前の追補（席数をそのまま初期値にする）を、席種類（`seatType`）に応じた固定値
    に差し替えた——**テーブル席は2名、カウンター席は1名**を初期値とする（実際の卓の席数
    （`seatCount`）は初期値には使わない）。カウンター席は1席ずつ卓を分けて登録する運用
    （`seatCount` は常に1。§4.4）のため、テーブル席とカウンター席とで「何名から始まる
    ことが多いか」の実態が異なる、という指摘を受けての修正。`FloorPage.tsx` の表示のみの
    変更（バックエンドは変更なし）。
  - 2026-09-18 追補（画面表示：「席種類」を「卓種類」に統一。FR-B02・FR-E01）：卓管理画面
    （`/settings/tables`）・注文管理（`/floor`）の絞り込み・入力項目のラベル「席種類」を
    「卓種類」に変更した（値自体はテーブル／カウンターのまま。内部の型名 `SeatType`・API
  ...
  - 2026-09-18 追補（注文管理画面：会計開始ボタンの位置と名称を変更。FR-G01）：会計を始める
    ボタンを、画面下部（追加注文セクションの下）から「現在の注文」見出しの右隣に移動し、
    ボタン名称を「会計を始める」から「会計処理へ」に変更した（`FloorPage.tsx` の表示のみの
    変更。バックエンドは変更なし）。
  - 2026-09-18 追補（卓一覧画面にも会計処理へのボタンを追加。FR-G01）：注文管理の卓一覧
    （卓ボード）から、卓の注文画面を経由せず直接会計画面へ進めるよう、営業中（利用中／
    会計中）の卓それぞれに「会計処理へ」ボタンを追加した（`FloorPage.tsx`）。卓カード自体は
    従来どおりクリックで注文画面へ遷移し、ボタン側だけがクリックイベントを止めて会計画面へ
    直接遷移する。会計画面のヘッダー（卓番号表示）は卓の詳細取得（`GET .../table-sessions/
    {sessionId}`）を経ずに直接開く経路もできたため、`openCheckout` 側で必要なら卓セッション
    詳細もあわせて取得するよう変更した。バックエンドは変更なし。
  - 2026-09-18 追補（追加注文：選択中のカートを「追加注文」見出しの下に一覧表示。FR-E02）：
    従来はカートに追加した品目の数量表示がメニュー一覧内（カテゴリで絞り込まれる領域）に
    埋め込まれていたため、あるカテゴリで品目をカートに入れたあと別のカテゴリに絞り込みを
    切り替えると、その品目がカテゴリ絞り込みで画面から消えてしまい「何を追加しようとして
    いたか分からなくなる」不具合があった。「追加注文」見出しの直下、カテゴリ絞り込みより
    前に、カート内の品目だけを（絞り込みに関係なく）常に一覧表示する領域を追加した
    （`FloorPage.tsx`。数量の増減もここから行える）。バックエンドは変更なし。
  - 2026-09-18 追補（注文管理画面：注文送信ボタンの位置と名称を変更。FR-E02）：カート下部に
    あった「注文を送信する」ボタンを「追加注文」見出しの右隣に移動し、ボタン名称を「注文送信」
    に変更した（「現在の注文」見出しの右隣に置いた「会計処理へ」と同じ配置）。
    `FloorPage.tsx` の表示のみの変更（バックエンドは変更なし）。
  - 2026-09-18 追補（不具合修正：「現在の注文」隣の「会計処理へ」ボタンが通信エラーになる。
    FR-G01）：注文管理を一通り実機確認したところ、卓一覧からではなく注文画面（「現在の注文」
    見出し隣）の「会計処理へ」ボタンからは会計画面へ進めず、「サーバーとの通信に失敗しました」
    と表示される不具合が見つかった。卓一覧に「会計処理へ」を追加した際（直前の追補）に
    `openCheckout` を「任意で `sessionId` を受け取れる」関数に変更したが、注文画面側のボタンは
    `onClick={openCheckout}` と関数をそのまま渡す書き方のままだったため、クリック時に
    ブラウザが渡す `MouseEvent` オブジェクトが `sessionId` 引数として渡ってしまい
    （`/table-sessions/[object%20Object]/checks` という不正なURLでリクエストされていた）、
    バックエンドに届く前にCORSエラーとして失敗していた。`onClick={() => openCheckout()}`
  ...
  - 2026-09-18 追補（注文管理：現在の注文に「注文状態で絞り込み」を追加。FR-E07）：「現在の注文」
    ...
    （バックエンドは変更なし）。
  - 2026-09-18 追補（注文管理：現在の注文の明細カードを注文状態で色分け。FR-E07）：「現在の注文」
    の各カードの背景色を注文状態ごとに変えた（未提供＝白、調理中＝薄黄色、提供済み＝薄緑、
    取消＝薄灰色、却下＝薄赤）。会計画面の明細一覧は同じ表示関数（`lineCardStyle`）を流用
    しているが、こちらには注文状態の概念が無いため既定の白背景のまま変わらないことを確認した。
    `FloorPage.tsx` の表示のみの変更（バックエンドは変更なし）。
  - 2026-09-19 追補（注文管理：「現在の注文」「追加注文」の2エリアを見た目で区別。FR-E02・E07）：
    注文画面で「現在の注文」と「追加注文」が同じ白背景で並んでいて境目が分かりにくかったため、
    両エリアをそれぞれ枠付きの箱で囲み、背景色を変えて区別できるようにした。「現在の注文」は
    グレー系の背景（`currentOrderBoxStyle`）、「追加注文」はカート内品目の表示（`#eef6ff` の
    青系配色）と揃えた薄い青背景＋青枠（`additionalOrderBoxStyle`）とした。取消理由入力フォームは
    「現在の注文」の箱の内側（対象の明細を取消す操作のため）にそのまま残した。`FloorPage.tsx` の
  ...
  - 2026-09-19 追補（予約もキッチンをホールと同じ権限に。FR-C01）：上記に続き、予約の登録・
    変更・キャンセルについてもキッチンをホールと同じ権限にした。`StoreAccessGuard#requireCanManageReservations`
    の許可ロールに `KITCHEN` を追加し、`ReservationService#list`（`storeId` 省略時の横断表示の
    可否判定）も同様に改めた。フロントは `ReservationsPage.tsx` の `canEditStore` に `KITCHEN`
    を追加。ホーム画面に「予約」の入口を表示するため `app_feature_role` に
    `(reservations, KITCHEN)` を追加（`V20__reservations_kitchen_access.sql`）。モバイル
    オーダー注文の受理・却下は対象外のまま変更していない。§6.3 の予約APIの説明も
    「店長・ホール」から「店長・ホール・キッチン」に更新した。
  - 2026-09-19 追補（不具合修正：注文管理・会計の閲覧がURL直打ちで誰でもできてしまう。
    FR-E01〜E03・FR-G01〜G05,G06,G07,G07b,G10,G11）：`02_requirements.md` §3.2の権限マトリクスは
    「卓のオープン／クローズ」「注文の入力・数量変更・取消」「会計の確定」「会計の取消・返金・
    値引き」を経営管理者・店長・ホール（・キッチン）のみ◯としており、バイト等その他ロールは
    空欄＝不可の意図だったが、実装では卓セッション・会計の閲覧系メソッド
    （`TableSessionService#listActive`／`#get`、`CheckoutService#listChecks`／`#getCheck`）が
    店舗設定・卓マスタ等と共通の緩い`StoreAccessGuard#requireCanView`
    （テナント内なら閲覧はロール問わず可、というのは店舗設定等マスタ情報向けの意図的な仕様。
    §2.5参照）を流用していたため、ホーム画面に「注文管理」が出ないロール（バイト等）でも
    URLを直接指定すれば卓・注文・会計の中身を閲覧できてしまっていた。上記4メソッドの権限判定を
    `requireCanView`から`StoreAccessGuard#requireCanManageFloor`（操作系と同じ権限）に変更し、
    閲覧も操作と同じロール・店舗所属の範囲に制限した。店舗設定・卓マスタ・決済手段・営業日等、
    他の`requireCanView`利用箇所（意図的にテナント内で閲覧を制限しない設計。§2.5参照）は対象外で
    変更していない。実機でバイト・キッチンの検証用アカウントを作成し、
    `GET /api/v1/stores/{storeId}/table-sessions`がバイトで403・キッチンで200になることを確認した
    （検証用アカウントは確認後に削除）。バックエンドの既存テスト65件は変更なし・全通過。
  - 2026-09-19 追補（不具合修正：無効なメニュー項目でも「販売中にする」ボタンが押せてしまう。
    FR-D01・D03）：メニュー編集画面で、無効な項目・無効なカテゴリを「保存する」前の一時的な
    状態（フォーム上でチェックを外しただけ、カテゴリを選び直しただけの未保存の状態）にしても、
    販売状況の切替ボタン（`SalesStatusButtons`）は保存済みの値をもとに全状態への切替ボタンを
    無条件に表示していたため、「有効にする」のチェックを外した直後でも見た目上「販売中にする」
    ボタンが押せてしまい、実際押すと（保存済みのメニュー項目自体はまだ有効なままなので）
    バックエンドは正しく受理してしまう、という紛らわしい状態になっていた（バックエンドの
    `MenuService#updateSalesStatus`自体は無効な項目への提供停止以外の変更を元々正しく拒否して
    おり、データの不整合は発生していなかった）。`SalesStatusButtons`に`canEnable`propを追加し、
    一覧画面では保存済みの`item.active`／カテゴリの有効状態、メニュー編集画面では**未保存の
    フォームの入力値**（`itemForm.active`とカテゴリのプルダウン選択）を見て、無効な場合は
    「提供停止にする」以外のボタンを出さないようにした。これにより「有効にする」のチェックを
    外した瞬間に、保存前でも「提供停止にする」以外の選択肢が消えるようになった。
  ...
  - 2026-09-19 追補（メニュー編集画面：「販売状況」の保存を「保存する」ボタンに一本化。
    ...
    テスト65件は変更なし・全通過。
  - 2026-09-19 追補（不具合修正：無効なメニュー項目を保存した後も「提供停止にしてください」の
    警告が消えない。FR-D01・D03）：上記の一本化対応の直後、無効化して保存したメニュー項目を
    再度開くと、既に「提供停止」で保存済み・矛盾のない状態にもかかわらず「メニューが無効の
    場合、設定する販売状況は「提供停止」にしてください。」という警告が表示され続ける不具合が
    見つかった。警告の表示条件が「項目・カテゴリが無効かどうか」だけで判定されており、実際に
    選択中の販売状況が既に「提供停止」で問題ない場合も区別せず出していたのが原因。表示条件に
    「選択中の販売状況がまだ『提供停止』になっていない」を追加し、既に矛盾がない場合は警告を
    出さないようにした（`itemForm.salesStatus !== 'SUSPENDED'` を追加）。`MenuManagementPage.tsx`
    の表示のみの変更（バックエンドは変更なし）。実機で報告の状況（「たこわさ」を無効化・提供
    停止で保存済み）を再現し、編集画面を開いても警告が出なくなったことを確認した。
