...
新しい居酒屋の会社と契約したら、その会社と最初の経営管理者アカウントを登録する。
...
2. **Body** タブの5項目を今回の値に書き換える。
   ...
   | `ownerName` | 経営管理者の氏名 | 必須 |
   | `ownerEmail` | 経営管理者のメールアドレス | 必須。ログインIDになる |
   | `password` | 経営管理者の初期パスワード | 8文字以上、英字と数字を両方含む |
3. **Send** を押す。
4. 右側のレスポンスを確認する。

   | ステータス | 意味 | 対応 |
   |---|---|---|
   | `201 Created` | 作成成功。本文に `{ "companyCode": "..." }` | 経営管理者へ「`<companyCode>.<サービスドメイン>` から、メールアドレスと初期パスワードでログイン」と案内 |
...
- 作成される経営管理者は全店権限（`role = OWNER`、特定店舗に紐づかない）。店舗やスタッフは経営管理者がログイン後に登録する。
