  ...
  return (
    <div style={{ maxWidth: '960px', margin: '40px auto', padding: '20px', textAlign: 'left' }}>
      ...
      <p style={{ color: '#666', fontSize: '13px' }}>
        {isManager ? (
          <>
            自分の所属店舗に関する店舗設定・決済手段設定・卓・営業日・予約の変更などの操作を、
            期間・操作種別・実行者で検索できます（ログイン履歴やユーザー権限変更など店舗に紐づかない
            操作、他店舗の操作は表示されません）。
          </>
        ) : (
          <>
            ログイン・パスワード変更・ユーザー権限変更・店舗設定/決済手段設定・卓・営業日・予約の
            変更などの操作を、期間・店舗・操作種別・実行者で検索できます（FR-J04）。
          </>
        )}
      </p>
